...
import { accessedDynamicData, createPostponedAbortSignal, formatDynamicAPIAccesses, isPrerenderInterruptedError, createDynamicTrackingState, createDynamicValidationState, trackAllowedDynamicAccess, throwIfDisallowedDynamic, consumeDynamicAccess } from './dynamic-rendering';
...
import { ServerPrerenderStreamResult, processPrelude } from './app-render-prerender-utils';
...
function App({ reactServerStream, preinitScripts, clientReferenceManifest, ServerInsertedHTMLProvider, ServerInsertedMetadataProvider, gracefullyDegrade, nonce }) {
    preinitScripts();
...
// consistent for now.
function ErrorApp({ reactServerStream, preinitScripts, clientReferenceManifest, ServerInsertedMetadataProvider, ServerInsertedHTMLProvider, gracefullyDegrade, nonce }) {
    preinitScripts();
    const response = React.use(useFlightStream(reactServerStream, clientReferenceManifest, nonce));
    const initialState = createInitialRouterState({
        // This is not used during hydration, so we don't have to pass a
 ...
 */ async function spawnDynamicValidationInDev(resolveValidation, tree, ctx, isNotFound, clientReferenceManifest, requestStore) {
    ...
    const initialServerPayload = await workUnitAsyncStorage.run(initialServerPrerenderStore, getRSCPayload, tree, ctx, isNotFound);
    ...
    const finalAttemptRSCPayload = await workUnitAsyncStorage.run(finalServerPrerenderStore, getRSCPayload, tree, ctx, isNotFound);
    ...
    try {
        const prerender = require('react-dom/static.edge').prerender;
        let { prelude: unprocessedPrelude } = await prerenderAndAbortInSequentialTasks(()=>workUnitAsyncStorage.run(finalClientPrerenderStore, prerender, /*#__PURE__*/ _jsx(App, {
                ...
                onError: (err, errorInfo)=>{
                    if (isPrerenderInterruptedError(err) || finalClientController.signal.aborted) {
                        ...
                        if (typeof componentStack === 'string') {
                            trackAllowedDynamicAccess(workStore.route, componentStack, dynamicValidation);
                        }
                        return;
                    }
                    return getDigestForWellKnownError(err);
        ...
        resolveValidation(/*#__PURE__*/ _jsx(LogSafely, {
            fn: throwIfDisallowedDynamic.bind(null, workStore, preludeIsEmpty, dynamicValidation, serverDynamicTracking, clientDynamicTracking)
        }));
    } catch (thrownValue) {
        // Even if the root errors we still want to report any dynamic IO errors
        // that were discovered before the root errored.
        const preludeIsEmpty = true;
        let loggingFunction = throwIfDisallowedDynamic.bind(null, workStore, preludeIsEmpty, dynamicValidation, serverDynamicTracking, clientDynamicTracking);
...
async function prerenderToStream(req, res, ctx, metadata, tree) {
    ...
    try {
       ...
       */ // Prerender controller represents the lifetime of the prerender.
            ...
            // detected invalid dynamic usage in the initial prerender phase.
            if (workStore.invalidDynamicUsageError) {
                if (isUseCacheTimeoutError(workStore.invalidDynamicUsageError) && allowEmptyStaticShell) {
            ...
            const finalAttemptRSCPayload = await workUnitAsyncStorage.run(finalServerPrerenderStore, getRSCPayload, tree, ctx, res.statusCode === 404);
            ...
            let dynamicValidation = createDynamicValidationState();
            const prerender = require('react-dom/static.edge').prerender;
            let { prelude: unprocessedPrelude, postponed } = await prerenderAndAbortInSequentialTasks(()=>workUnitAsyncStorage.run(finalClientPrerenderStore, prerender, /*#__PURE__*/ _jsx(App, {
                    reactServerStream: reactServerResult.asUnclosingStream(),
                    preinitScripts: preinitScripts,
                    clientReferenceManifest: clientReferenceManifest,
                    ServerInsertedHTMLProvider: ServerInsertedHTMLProvider,
            ...
            if (!allowEmptyStaticShell) {
                throwIfDisallowedDynamic(workStore, preludeIsEmpty, dynamicValidation, serverDynamicTracking, clientDynamicTracking);
            }
            const getServerInsertedHTML = makeGetServerInsertedHTML({
                polyfills,
                renderServerInsertedHTML,
