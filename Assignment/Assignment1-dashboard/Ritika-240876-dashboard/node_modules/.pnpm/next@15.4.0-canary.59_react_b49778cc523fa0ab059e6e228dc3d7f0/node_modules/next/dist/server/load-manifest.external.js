...
function loadManifestFromRelativePath({ projectDir, distDir, manifest, shouldCache, cache, skipParse, handleMissing }) {
    try {
        return loadManifest((0, _path.join)(/* turbopackIgnore: true */ projectDir, distDir, manifest), shouldCache, cache, skipParse);
    } catch (err) {
        if (handleMissing) {
