...
// //     const userId = localStorage.getItem('userId');  // get userId stored after login
// //     if (!userId) {
// //       console.error('No user ID found. Redirect to login or show message.');
// //       setLoading(false);
// //       return;
// //     }
...
// useEffect(() => {
//   const fetchProfile = async () => {
//     const userId = localStorage.getItem('userId');  // 👈 get user id stored after login
//     if (!userId) {
//       console.error('No user ID found. Redirect to login or show a message.');
//       setLoading(false);
//       return;
//     }

//     try {
//       const res = await fetch(`/api/user/${userId}`);  // 👈 dynamically fetch user profile
//       if (!res.ok) throw new Error('Failed to fetch profile');
//       const data = await res.json();
//       setProfile(data);
//     } catch (error) {
...
export default function ProfilePage() {
  ...
  const shareViaNative = async () => {
    ...
    if (navigator.share) {
      ...
      alert('Web Share not supported in your browser');
    }
  };
      ...
      <main className="flex flex-col gap-8 sm:gap-10 items-center w-full mx-auto max-w-4xl">
        ...
        <div className="w-full text-center sm:text-left">
          <div className="flex flex-col sm:flex-row items-center gap-4 sm:gap-6 mb-6 sm:mb-8">
            <div className="relative w-24 h-24 sm:w-32 sm:h-32 rounded-full overflow-hidden border-4 border-white dark:border-gray-800 shadow-lg ring-2 ring-blue-200 dark:ring-blue-800">
              <Image
                src={profile.user.profile_image || "/default-avatar.png"}
                alt="Profile"
                width={128}
                height={128}
                className="object-cover w-full h-full"
          ...
          <div className="bg-white dark:bg-gray-800 p-6 rounded-b-lg shadow-sm border border-gray-100 dark:border-gray-700">
            {profile.contribution ? (
              <div className="space-y-4">
                <p className="text-gray-700 dark:text-gray-300">{profile.contribution.review}</p>
                {profile.contribution.image_url && (
                  <Image 
                    src={profile.contribution.image_url}
                    alt="Contribution Spot"
                    width={400}
                    height={300}
        ...
        <div className="w-full">
          ...
          <div className="bg-white dark:bg-gray-800 p-6 rounded-b-lg shadow-sm border border-gray-100 dark:border-gray-700">
            {profile.spot ? (
              <div className="space-y-4">
                <p className="text-gray-700 dark:text-gray-300">{profile.spot.description}</p>
                {profile.spot.image_url && (
                  <Image 
                    src={profile.spot.image_url}
                    alt="Favorite Spot"
              ...
              <p className="text-gray-500 dark:text-gray-400">No favorite spot added.</p>
            )}
          </div>
        </div>
